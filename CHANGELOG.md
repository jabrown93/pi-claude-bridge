...
- **Fix: a mid-turn prompt rebuild no longer fails the turn (single-agent sessions, `glla`/pi-subagents)** — when pi rebuilt the system prompt outside `before_agent_start` (fresh skill or resource discovery rewrites the skills section, a late-registered tool rewrites the tools list, or the pi-subagents refinement overlay rewrites the sub-agent envelope), the rebuilt prompt matched no capture key and embedded none, so `resolveOrDerive` threw `no capture for this N-char system prompt …` and the turn/worker died. It now recovers the owning agent's recorded parts through a churn-invariant anchor — its `<project_context>` block, its custom/replace prompt (or a sub-agent's own slices of it), or a stable append — ranked to the most specific matching node so a sub-agent recovers itself rather than its embedded parent, and surfaces the recovery through a durable `prompt-capture-recovered` diagnostic. Because a `<project_context>` block is shared byte-for-byte by every agent in a project, recovery fails closed (throws as before) when a match could belong to a sibling it cannot disambiguate, rather than forwarding the wrong agent's context.
